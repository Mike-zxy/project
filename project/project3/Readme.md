...
## 1. 实验目标：
实现一个 Circom 电路，用于验证输入为某个哈希原像（私有输入）时，其 Poseidon2 哈希值（公开输入）是否正确。使用参数 $(n=256, t=3, d=5)$ 或 $(n=256, t=2, d=5)$。最后使用 Groth16 生成 zkSNARK 证明。

以下是用 Circom 实现 Poseidon2 哈希算法电路的完整方案：
